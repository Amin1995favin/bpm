...
################### login_warehouse_iran ###################

    def test01_login_warehouse_iran(self):
        self.driver.get(base_url)
        login = LoginPage(driver=self.driver)
        login.enter_login_username(warehouse_iran)
        login.enter_login_btn_submit_next()
        login.enter_login_password(code_no_admin)
        login.enter_login_btn_submit()
        print("با موفقیت وارد پنل انبار دار ایران شد")
        print("############################################")

################### warehouse_iran_report_check ###################

    def test02_warehouse_iran_report_check(self):
        report = Report(driver=self.driver)
        report.enter_report()
        report.enter_report_manager_check()
        report.enter_len_warehouse_iran_report_check()
        print("############################################")

################### warehouse_iran_reports ###################

    def test03_warehouse_iran_reports(self):
        report = Report(driver=self.driver)
        report.enter_report()
...
Mongodb.run_tests_and_insert_into_mongodb(Test_bpm, 'bpm')
